...
let
  ...
  jhide = build-asdf-system rec {
    ...
    buildScript = pkgs.writeText "build-jhide.lisp" ''
      (load "${self.jhide.asdfFasl}/asdf.${self.jhide.faslExt}")
    ...
    flags = ''
      --noinform --non-interactive --end-toplevel-options
    '';
